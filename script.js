class ActividadManager {
  ...
  function mostrarDatosAlmacenados() {
    const datosPrevios = JSON.parse(localStorage.getItem("datos"));
    const datosTableBody = document.getElementById("datos-almacenados");

    datosTableBody.innerHTML = ""; // Limpiamos el contenido existente en la tabla
    ...
    if (datosPrevios) {
      ...
      const row = document.createElement("tr");
      row.innerHTML = `<td colspan="4">No hay datos almacenados.</td>`;
      datosTableBody.appendChild(row);
    }
  }
